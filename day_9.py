...
to a new part of the network. After briefly scanning it for interesting files,
...
and repeat them 2 times. Then, continue reading the file after the repeated data.
...
def main():
    ...
    with open('day_9_data.txt', 'r') as data:

        counter = i = to_delete = 0
        number1 = number2 = ''
        letters = list(data.read().replace('\n',''))

        while True:
            if letters[i] == '(':
                i += 1
                number1 = ''
                while letters[i].isdigit():
                    number1 += letters[i]
                    i += 1
                i += 1
                number2 = ''
                while letters[i].isdigit():
                    ...
                    i += 1
                i += 1
                to_delete = i + int(number1)
                counter += int(number1)*int(number2)
                number1 = number2 = ''

                letters = list(letters[to_delete:])
                i = 0

            if len(letters) < 1:
                break

            while not letters[0].isdigit() and letters[0] != '(':
                letters = letters[1:]
